readData<-function(){
    ...
    dt$Global_reactive_power<-as.numeric(gsub("?",0,dt$Global_reactive_power, fixed = TRUE))
    #dt$Global_reactive_power[is.na(dt$G)] <- 0 
    ...
    dt[is.na(dt$Global_reactive_power)]<-0
    dt
}
